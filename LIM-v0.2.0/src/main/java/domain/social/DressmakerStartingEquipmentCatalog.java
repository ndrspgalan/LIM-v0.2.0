...
public final class DressmakerStartingEquipmentCatalog {
 private DressmakerStartingEquipmentCatalog(){}
 public static CanonicalStartingEquipment equipment(Subprofession s,CharacterClass c){
  ...
  if(s.profession()!=Profession.DRESSMAKER)throw new IllegalArgumentException("Sólo DRESSMAKER.");
  if(DressmakerCanonicalProfiles.isDeprecated(s,c))throw new IllegalArgumentException("Perfil  deprecated: "+s+" / "+c);
  CanonicalStartingEquipment base=switch(s){
   case WORK_TAILOR -> switch(c){
    case INTELECTUAL -> eq(precise(true),List.of("Pan","Fruta","Odre","Caja del Artesano"),List.of(),55,List.of(ArmorMaterial.CLOTH,ArmorMaterial.CLOTH),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    case APODERADO -> eq(precise(false),List.of("Pan","Fruta","Odre","Caja del Artesano"),List.of(),60,List.of(ArmorMaterial.CLOTH,ArmorMaterial.CLOTH),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    default -> throw new IllegalArgumentException();};
   case PRECISION_PATTERNMAKER -> switch(c){
    case ESPECIALISTA -> eq(precise(false),List.of("Bizcocho","Fruta","Odre","Caja del Artesano"),List.of(),70,List.of(ArmorMaterial.CLOTH,ArmorMaterial.CLOTH,ArmorMaterial.CLOTH),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    default -> throw new IllegalArgumentException();};
   case SALON_DRESSMAKER -> switch(c){
    case HERALDO -> eq(salon(false),List.of("Bizcocho","Fruta","Odre","Caja del Artesano","ESPEJO DE BOLSILLO"),List.of(),105,List.of(ArmorMaterial.CLOTH,ArmorMaterial.CLOTH),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    default -> throw new IllegalArgumentException();};
