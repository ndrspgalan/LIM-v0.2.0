...
public final class TannerCanonicalProfiles {
    ...
    private TannerCanonicalProfiles(){}
    ...
    public static CanonicalSubprofessionProfile profile(Subprofession s,CharacterClass c){
        ...
        if(by==null)throw new IllegalArgumentException("Subprofesión fuera de TANNER: "+s);
    ...
    public static Map<CharacterClass,CanonicalSubprofessionProfile> profiles(Subprofession s){
        var p=DATA.get(Objects.requireNonNull(s));if(p==null)throw new IllegalArgumentException("Subprofesión fuera de TANNER: "+s);return p;
    }
    ...
    public static boolean isDeprecated(Subprofession s,CharacterClass c){
        ...
        if(s.profession()!=Profession.TANNER)throw new IllegalArgumentException("Profesión incorrecta: "+s);
    ...
    private static Map<Subprofession,Set<CharacterClass>> active(){
        ...
        m.put(Subprofession.HIDE_PREPARER,Set.of(CharacterClass.LUCHADOR));
        m.put(Subprofession.INDUSTRIAL_TANNER,Set.of(CharacterClass.INTELECTUAL));
        m.put(Subprofession.LEATHER_FINISHER_GRADER,Set.of(CharacterClass.ESPECIALISTA));
        return Map.copyOf(m);
    }
    private static Map<Subprofession,Map<CharacterClass,CanonicalSubprofessionProfile>> build(){
        ...
        add(all,Subprofession.HIDE_PREPARER,CharacterClass.LUCHADOR,sheet(31,34,14,43,23,14,4,8,4),"Perfil canónico explícito  de HIDE_PREPARER / LUCHADOR: la hoja deriva de la función artesanal, la biografía y el sello innato; ninguna fórmula automática de afinidad participa en su construcción.");
        add(all,Subprofession.INDUSTRIAL_TANNER,CharacterClass.INTELECTUAL,sheet(24,27,26,18,26,39,6,14,14),"Perfil canónico explícito  de INDUSTRIAL_TANNER / INTELECTUAL: la hoja deriva de la función artesanal, la biografía y el sello innato; ninguna fórmula automática de afinidad participa en su construcción.");
        add(all,Subprofession.LEATHER_FINISHER_GRADER,CharacterClass.ESPECIALISTA,sheet(21,24,25,11,43,32,6,15,10),"Perfil canónico explícito  de LEATHER_FINISHER_GRADER / ESPECIALISTA: la hoja deriva de la función artesanal, la biografía y el sello innato; ninguna fórmula automática de afinidad participa en su construcción.");
        if(!all.keySet().equals(ACTIVE.keySet()))throw new IllegalStateException("taxonomía TANNER incompleta.");
