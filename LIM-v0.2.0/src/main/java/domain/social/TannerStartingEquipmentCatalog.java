...
public final class TannerStartingEquipmentCatalog {
 private TannerStartingEquipmentCatalog(){}
 public static CanonicalStartingEquipment equipment(Subprofession s,CharacterClass c){
  ...
  if(s.profession()!=Profession.TANNER)throw new IllegalArgumentException("Sólo TANNER.");
  if(TannerCanonicalProfiles.isDeprecated(s,c))throw new IllegalArgumentException("Perfil  deprecated: "+s+" / "+c);
  CanonicalStartingEquipment base=switch(s){
   case HIDE_PREPARER -> switch(c){
    case LUCHADOR -> eq(work(true),List.of("Pan","Cecina","Odre","Amadou","Pedernal","Caja del Artesano","Apósito de musgo de turbera"),List.of(),28,List.of(ArmorMaterial.HARDENED_LEATHER),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    case INDOMITO -> eq(work(true),List.of("Pan","Cecina","Frutos secos","Odre","Caja del Artesano","Emplasto de milenrama"),List.of(),24,List.of(ArmorMaterial.HARDENED_LEATHER),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    default -> throw new IllegalArgumentException();};
   case INDUSTRIAL_TANNER -> switch(c){
    case INTELECTUAL -> eq(precise(true),List.of("Pan","Fruta","Odre","Caja del Artesano"),List.of(),52,List.of(ArmorMaterial.HARDENED_LEATHER,ArmorMaterial.HARDENED_LEATHER),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    case APODERADO -> eq(precise(false),List.of("Pan","Fruta","Odre","Caja del Artesano","Apósito de musgo de turbera"),List.of(),58,List.of(ArmorMaterial.HARDENED_LEATHER,ArmorMaterial.HARDENED_LEATHER),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    default -> throw new IllegalArgumentException();};
   case LEATHER_FINISHER_GRADER -> switch(c){
    case ESPECIALISTA -> eq(precise(false),List.of("Pan","Fruta","Odre","Caja del Artesano","Piedra de afilar"),List.of(),64,List.of(ArmorMaterial.HARDENED_LEATHER),List.of(InventoryCompartmentType.BACKPACK),Optional.empty());
    default -> throw new IllegalArgumentException();};
